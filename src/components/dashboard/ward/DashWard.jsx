...
export default function DashWard() {
  const [data, setData] = useState([]);
  const [filterText, setFilterText] = useState("");

  const fetchData = async () => {
    try {
      const response = await fetch(`${envVar.api_url}/ward`);
  ...
  const filteredData = data.wards?.filter((item) => {
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold py-4">Quản lý xã phường</h1>
